# План реализации загрузчика пятиминутных данных MEXC
...
**Цель:** создать скрипт, который скачивает пятиминутные свечи BTCUSDT с MEXC начиная с даты из `settings.yaml`, сохраняет их в годовые SQLite-базы и при повторном запуске продолжает загрузку с новой свечи после последней уже сохраненной.

**Архитектура:** загрузчик будет состоять из небольших тестируемых частей в `src/pj22_btc`: чтение настроек, клиент MEXC для kline-данных, годовое SQLite-хранилище и orchestration-логика докачки. `scripts/download_mexc_klines.py` будет тонкой CLI-оберткой. Unit-тесты будут использовать fake-клиент и временные SQLite-директории, поэтому они не зависят от живого API MEXC.
...
Добавить тесты для выбора годового DB-файла, идемпотентной записи в SQLite, расчета cursor-а для докачки и записи свечей в разные годовые базы.
...
- Изменить: `.gitignore`
- Изменить: `README.md`
- Создать: `data/mexc/klines/BTCUSDT/5m/.gitkeep`

- [ ] **Шаг 1: Добавить конфигурацию проекта**

`settings.yaml` должен содержать `start_date: 2025-09-01`, `symbol: BTCUSDT`, `interval: 5m` и директорию для годовых SQLite-файлов. `.gitignore` должен исключать SQLite DB-файлы и служебные sidecar-файлы SQLite.

- [ ] **Шаг 2: Добавить CLI-обертку**

Скрипт должен читать `settings.yaml`, вызывать `sync_klines` и печатать короткую сводку: сколько строк скачано, сколько вставлено, с какого cursor-а началась докачка, какая последняя свеча обработана и где лежат годовые DB-файлы.
